...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	[[0,format["%1 ton %2 est en train d'être saisie définitivement par la Gendarmerie.",(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	...
	if(player distance _vehicle > 10) exitWith {hint "Saisie annulée"; life_action_inUse = false;};
	...
	if((count crew _vehicle) == 0) then
	{
		...
		[[_vehicle,true,player],"TON_fnc_vehicleStoreCop",DB_Dest,false] spawn life_fnc_MP;
