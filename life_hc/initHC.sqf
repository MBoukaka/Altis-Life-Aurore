...
if(HC_Debug_Queries) then {
    _handle = [] spawn HC_fnc_debugQueries;
    waitUntil {scriptDone _handle};
    [] spawn {
        while {true} do {
            _queryCount = HC_DBInsertCounter + HC_DBSelectCounter;
            uisleep (HC_Status_Time * 60);
            waitUntil {uisleep 0.2; HC_DBInsertCounter + HC_DBSelectCounter > _queryCount;};
            [("")] call HC_Logging;
            [("____________________________________________________________________________")] call HC_Logging;
            [(format["extDB: Query counter: a info message which appears every %1 seconds",(HC_Status_Time * 60)])] call HC_Logging;
            [("----------------")] call HC_Logging;
            [(format["MySQL Total queries: %1",(HC_DBInsertCounter + HC_DBSelectCounter)])] call HC_Logging;
            [(format["MySQL Total queries average time for one query: %1 ms",((HC_DBInsertTime + HC_DBSelectTime) / (HC_DBInsertCounter + HC_DBSelectCounter))])] call HC_Logging;
            [("----------------")] call HC_Logging;
            [(format["MySQL Insert/Update queries in total: %1",HC_DBInsertCounter])] call HC_Logging;
            [(format["MySQL Insert/Update average time for one query: %1 ms",(HC_DBInsertTime / HC_DBInsertCounter)])] call HC_Logging;
            [("----------------")] call HC_Logging;
            [(format["MySQL Select queries in total: %1",HC_DBSelectCounter])] call HC_Logging;
            [(format["MySQL Select average time for one query: %1 ms",(HC_DBSelectTime / HC_DBSelectCounter)])] call HC_Logging;
            [("____________________________________________________________________________")] call HC_Logging;
            [("")] call HC_Logging;
